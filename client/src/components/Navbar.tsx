...
export const Navbar: FC<NavbarProps> = ({
  ...
  // Real user metadata when logged in
  const displayName = user?.name || '';
  const displayRole = user?.role === 'HR_MANAGER'
    ? (isAr ? 'مدير توظيف' : 'HR Manager')
    : (isAr ? 'مسؤول توظيف' : 'Recruiter');
    ...
    <header className="sticky top-0 z-50 backdrop-blur-xl bg-[#070A14]/95 border-b border-white/[0.08] mb-6 transition-all">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex items-center justify-between h-16">
        ...
        <div className="flex items-center gap-3">
          {/* Logo Mark & Wordmark */}
          <div
            onClick={() => setActiveTab(isLoggedIn ? 'pipeline' : 'careers')}
        ...
        {/* ================================================================= */}
        {/* ZONE 2: HR NAVIGATION TABS (Hidden completely from regular users) */}
        {/* ================================================================= */}
        {isLoggedIn ? (
          ...
          <div className="hidden sm:block text-xs text-[#5A6478]">
            {isAr ? 'من التقديم إلى التعيين، في مسار واحد' : 'One track from apply to hire'}
          </div>
        )}

        {/* ================================================================= */}
        {/* ZONE 3: UTILITIES & AUTHENTICATED USER MENU */}
        {/* ================================================================= */}
        <div className="flex items-center gap-3">
          {/* Public Careers Portal Link (Only shown to HR when inside workspace) */}
          {isLoggedIn && (
            <button
              type="button"
              onClick={() => setActiveTab('careers')}
              title={isAr ? 'معاينة بوابة الوظائف العامة' : 'View Public Careers Portal'}
          ...
          {/* Outbox Modal Trigger (Only for authenticated HR) */}
          {isLoggedIn && (
            <button
              type="button"
              onClick={onOpenOutbox}
              title={isAr ? 'صندوق البريد الصادر' : 'Email Outbox'}
                ...
                <div className="absolute top-full mt-2 end-0 z-50 w-64 bg-[#0E1524] border border-white/10 rounded-2xl shadow-2xl p-4 space-y-3 backdrop-blur-xl animate-in fade-in duration-150 text-start">
                  ...
                  <div className="space-y-1">
                    ...
                    >
                      ...
                      <span>{isAr ? 'الملف الشخصي' : 'User Profile'}</span>
                    </button>
                    ...
                    >
                      ...
                      <span>{isAr ? 'منشئ نماذج التقديم' : 'Application Form Builder'}</span>
                    </button>
                    ...
                    >
                      ...
                      <span>{isAr ? 'صندوق البريد الصادر' : 'Email Outbox'}</span>
                    </button>
                  </div>
